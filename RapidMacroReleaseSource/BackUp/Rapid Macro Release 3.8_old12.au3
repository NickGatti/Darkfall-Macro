...
;Rapid Group Sex BY Diminished Hail: Release Version 3.7
Opt("MustDeclareVars", 1)
MsgBox(0, "Version", "Rapid Group Sex BY Diminished Hail: Release Version 3.7")
...
Global $VMWareREG, $unsheathe, $HTMK, $guimode, $Checkbox4, $Checkbox5, $Checkbox6, $button3, $add1, $add2, $add3, $send, $state, $lbl3
...
GUICtrlSetState(-1, $GUI_DISABLE)
$Label3 = GUICtrlCreateLabel($lbl3, 390, 140, 140, 97)
...
Do
	...
	If $nMsg = $MenuItem2 Then
		...
		Exit
	EndIf
	If $nMsg = $MenuItem4 Then
	$lbl3 = "Author: Diminished Hail"
	GUISetState()
EndIf
		...
		For $i = 0 To 25
			Select
				Case GUICtrlRead($combobox) = $comboxarray[$i]
					...
					$Label3 = GUICtrlCreateLabel("Previous hokey is..." & $num, 390, 140, 140, 97)
			...
			For $i = 0 To 25
				Select
					Case GUICtrlRead($combobox) = $comboxarray[$i]
						...
						If $state = $GUI_CHECKED Then $add1 = "^"
						$state = GUICtrlGetState($Checkbox5)
						If $state = $GUI_CHECKED Then $add2 = "!"
						$state = GUICtrlGetState($Checkbox5)
						If $state = $GUI_CHECKED Then $add3 = "+"
						$send = $add1 & $add2 & $add3 & $regarray[$i]
						RegWrite("HKEY_LOCAL_MACHINE\SOFTWARE\DimsMacro", $send, "REG_SZ", $hotkey)
