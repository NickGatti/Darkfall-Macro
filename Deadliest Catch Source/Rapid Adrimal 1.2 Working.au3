...
Func drive()
	While 1
		...
		For $i = 3 To $totalUsed
			Do
				findFacing($i)
				$directionNeededToFaceLeft = $directionNeededToFace - 4
				$directionNeededToFaceRight = $directionNeededToFace + 4
				findDistance($i)
				If ($directionIamFacing > $directionNeededToFaceLeft And $directionIamFacing < $directionNeededToFaceRight) <> True And $currentDistance > 200000 Then
					facetowaypoint($i)
				EndIf
				ToolTip("Facing", 0, 0)
...
Func facetowaypoint($i)
	...
	Until ($directionIamFacing > $directionNeededToFaceLeft And $directionIamFacing < $directionNeededToFaceRight) Or $currentDistance < 200000
	Send("{a up}")
	Send("{d up}")
