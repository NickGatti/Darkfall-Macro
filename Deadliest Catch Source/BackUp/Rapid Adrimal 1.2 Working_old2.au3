...
Func drive()
	While 1
		Local $directionNeededToFaceAct, $directionIamFacingLeft, $directionIamFacingRight
		Send("{w down}")
		For $i = 3 To $totalUsed
			Do
				findFacing($i)
				$directionNeededToFaceAct = $directionIamFacing
				$directionIamFacingLeft = $directionNeededToFaceAct - 12
				$directionIamFacingRight = $directionNeededToFaceAct + 12
				findDistance($i)
				If ($directionNeededToFace > $directionIamFacingLeft And $directionNeededToFace < $directionIamFacingRight) <> True And $currentDistance > 90000 Then
					facetowaypoint($i)
				EndIf
				ToolTip("Facing", 0, 0)
...
Func facetowaypoint($i)
	...
	Until ($directionNeededToFace > $directionIamFacingLeft And $directionNeededToFace < $directionIamFacingRight) Or $currentDistance < 90000
	Send("{a up}")
	Send("{d up}")
