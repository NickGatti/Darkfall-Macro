	...
	For $i = 3 To $totalUsed
		Do
			...
			If Not ($headingM <= $faceDir And $headingP >= $faceDir) Then
				...
				Sleep(90)
				$faceDirAct = $heading
				If $faceDir > $faceDirAct And $faceDirAct < 180 Then
					$faceDirAct = $faceDirAct + 180
				EndIf
				If $faceDir < $faceDirAct And $faceDirAct > 180 Then
					$faceDirAct = $faceDirAct - 180
				EndIf
				If $faceDir < $faceDirAct Then
					MsgBox(0,"Left",$faceDirAct)
				ElseIf $faceDir > $faceDirAct Then
					MsgBox(0,"Right",$faceDirAct)
				EndIf
				Until ($headingM <= $faceDir And $headingP >= $faceDir)
			EndIf

			MsgBox(0,"Facing",$faceDirAct)
