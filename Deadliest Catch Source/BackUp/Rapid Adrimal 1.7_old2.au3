...
Func drive()
	...
	For $i = 3 To $totalUsed
		Do
			...
			$headingP = $faceDir + 15
			findDistance($i)
			If ($headingM <= $faceDir And $headingP >= $faceDir) Then ;And $currentDistance > 90000 Then
				facetowaypoint()
			EndIf
			MsgBox(0,"","IM AM FACING")
			ToolTip("Facing", 0, 0)
...
Func facetowaypoint()
				...
				Do
					...
					findDistance($i)
					$faceDirAct = $heading
					$headingLeft = $faceDirAct - 15
					$headingRight = $faceDirAct + 15
				...
				Until ($faceDir >= $headingLeft And $faceDir <= $headingRight) ;Or $currentDistance < 90000
				;Send("{a up}")
				;Send("{d up}")
EndFunc