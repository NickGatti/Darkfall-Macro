		...
		Do
			...
			If Not ($headingM <= $faceDir And $headingP >= $faceDir) Then
				Do
					...
					EndIf
					If $dirImGoing > 0 Then
					Send("{left}")
;### Tidy Error -> "else" is closing previous "do" on line 161
				Else
					Send("{right}")
;### Tidy Error -> "endif" is closing previous "do" on line 161
				EndIf
				;If $faceDir < 180 Then Send("{left down}")
				;ToolTip("Turning to Target", 0, 0)
				ToolTip("", 0, 0)
;### Tidy Error -> "until" is closing previous "if" on line 160
			Until ($headingM <= $faceDir And $headingP >= $faceDir)
			Send("{left up}")
			Send("{right up}")
;### Tidy Error -> "endif" is closing previous "do" on line 153
		EndIf
		ToolTip("Facing Target", 0, 0)
		Send("{w down}")
;### Tidy Error -> "until" is closing previous "for" on line 152
	Until $currentDistance < 25000
	Send("{w up}")
	ToolTip("Complete", 0, 0)
;### Tidy Error -> "next" is closing previous "func" on line 148
Next
