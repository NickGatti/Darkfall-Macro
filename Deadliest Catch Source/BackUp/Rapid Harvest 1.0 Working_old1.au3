...
Global $xParsed, $yParsed, $var, $i, $calcang, $heading, $faceDir, $faceDirAct, $positionX, $positionY
...
Func drive()
	...
	For $i = 3 To $totalUsed
		...
		Do
			findFacing($i)
			$faceDirAct = $heading
			$headingLeft = $faceDirAct - 8
			$headingRight = $faceDirAct + 8
			findDistance($i)
			If ($faceDir > $headingLeft And $faceDir < $headingRight) <> True And $currentDistance > 100000 Then
				facetowaypoint($i)
			EndIf
			ToolTip("Facing", 0, 0)
...
Func facetowaypoint($i)
	...
	Do
		...
		EndIf
		Sleep(50)
		ToolTip("Not Facing, need to face: " & $faceDir, 0, 0)
	Until ($faceDir > $headingLeft And $faceDir < $headingRight) Or $currentDistance < 100000
	Send("{w down}")
