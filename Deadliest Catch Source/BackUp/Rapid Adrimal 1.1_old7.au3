;WinActivate("Darkfall Online", "")
Global $xParsed, $yParsed, $var, $i
Global $Paused, $arrayLoc, $currentSpot = 2, $totalUsed = 2
Global $currentX, $currentY, $spotX, $spotY, $testX, $testY, $currentDistance, $savedDistance = 0
Global $arrayLoc, $totalUsed, $currentSpot, $xCurrentLoc, $yCurrentLoc, $xParsed, $yParsed
Global $spotNumber = 1
Global $spot[100]
$spot[0] = $currentSpot
$spot[1] = $totalUsed
;init()

HotKeySet("{Pause}", "TogglePause")
;HotKeySet("{DEL}", "SetPathingCoords")
;HotKeySet("{END}", "Drive")
HotKeySet("{HOME}", "getcurrentxy")

Func TogglePause()
	$Paused = Not $Paused
	While $Paused
		Sleep(100)
		ToolTip('Script is "Paused"', 0, 0)
	WEnd
	ToolTip("")
EndFunc   ;==>TogglePause

While 1
WEnd

Func init()
	Local $text, $var
	Run("loader.exe")
	Do
		Sleep(100)
	Until WinExists("Darkfall Tools")
	Do
		Sleep(250)
		$text = WinGetText("Darkfall Tools", "")
		$var = StringLeft($text, 13)
		ControlCommand("Darkfall Tools", "", "[Class:SysTabControl32; instance:1]", "TabRight", "")
	Until "Debug Console" = $var
EndFunc   ;==>init

Func getCurrentXY()
	$text = WinGetText("Darkfall Tools", "")
	$var = StringRight($text, 62)
	$result = StringInStr($var, ":")
	If $result Then
		$text = StringRight($text, 60)
		$x = StringLeft($text, 10)
		;MsgBox(0, "x", "(" & $x & ")")
		$text = StringRight($text, 50)
		$y = StringLeft($text, 10)
		;MsgBox(0, "y", "(" & $y & ")")
		$text = StringRight($text, 41)
		$z = StringLeft($text, 9)
		;MsgBox(0, "z", "(" & $z & ")")
		$text = StringRight($text, 31)
		$h = StringLeft($text, 4)
		;MsgBox(0, "h", "(" & $h & ")")
	EndIf
	If Not $result Then
	$x = StringLeft($var, 10)
	;MsgBox(0, "xparsed", "(" & $x & ")")
	$var = StringRight($text, 52)
	$y = StringLeft($var, 10)
	;MsgBox(0, "yparsed", "(" & $y & ")")
	$var = StringRight($text, 42)
	$z = StringLeft($var, 8)
	;MsgBox(0, "xparsed", "(" & $z & ")")
	$var = StringRight($text, 33)
	$h = StringLeft($var, 6)
	;MsgBox(0, "heading", "(" & $h & ")")
	EndIf
	$heading = Int($h)
	$xCurrentLoc = Int($x)
	$yCurrentLoc = Int($y)
	MsgBox(0, "heading", "(" & $heading & ")")
	MsgBox(0, "xCurrent", "(" & $xCurrentLoc & ")")
	MsgBox(0, "yCurrent", "(" & $yCurrentLoc & ")")
EndFunc   ;==>getCurrentXY

Func setPathingCoords()
	getCurrentXY()
	$totalUsed = $totalUsed + 1
	$spot[$totalUsed] = $xCurrentLoc & $yCurrentLoc
	$xParsed = $xCurrentLoc
	$yParsed = $yCurrentLoc
EndFunc   ;==>setPathingCoords

Func drive()
	findNearestCoord()
	ToolTip($arrayLoc, 0, 0)
EndFunc   ;==>drive

Func findNearestCoord()
	$runonce = True
	For $i = 3 To $totalUsed ; For Loop Beginning from $i to the Max Number of Arrary Vars
		getCurrentXY()
		parseLoc($spot[$i]) ; Parse arrary x y
		$testXLoc = Int($xCurrentLoc) - Int($xParsed) ; testX is the distance between the pathing loc (static) and the current loc
		$testYLoc = Int($yCurrentLoc) - Int($yParsed); testY is the distance between the pathing loc (static) and the current loc
		MsgBox(0, "xcurrent", $xCurrentLoc)
		MsgBox(0, "xparsed", $xParsed)
		MsgBox(0, "ycurrent", $yCurrentLoc)
		MsgBox(0, "yparsed", $yParsed)
		$currentDistance = Sqrt(($testXLoc *  $testXLoc) + ($testYLoc * $testYLoc))  ;This is an equation that solves the total distance of X and Y
		MsgBox(0, "Current Distance", $currentDistance)
		If $currentDistance < $savedDistance Or $runonce Then
			$savedDistance = $currentDistance
			$arrayLoc = $i
			$runonce = False
		EndIf
		MsgBox(0, "Saved Distance", $savedDistance & " Number is: " & $i)
	Next
EndFunc   ;==>findNearestCoord

Func parseLoc($loc)
	$xParsed = StringLeft($loc, 9)
	$var = StringRight($loc, 9)
	$yParsed = StringLeft($var, 9)
EndFunc   ;==>parseLoc